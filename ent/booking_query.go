...
func (bq *BookingQuery) QueryOffice() *OfficeQuery {
	query := (&OfficeClient{config: bq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := bq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (bq *BookingQuery) Clone() *BookingQuery {
	...
	return &BookingQuery{
		...
		withOffice: bq.withOffice.Clone(),
...
// WithOffice tells the query-builder to eager-load the nodes that are connected to
// the "office" edge. The optional arguments are used to configure the query builder of the edge.
func (bq *BookingQuery) WithOffice(opts ...func(*OfficeQuery)) *BookingQuery {
	query := (&OfficeClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (bq *BookingQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Booking, error) {
	...
	if query := bq.withOffice; query != nil {
		if err := bq.loadOffice(ctx, query, nodes, nil,
			func(n *Booking, e *Office) { n.Edges.Office = e }); err != nil {
			return nil, err
		}
	}
...
func (bq *BookingQuery) loadOffice(ctx context.Context, query *OfficeQuery, nodes []*Booking, init func(*Booking), assign func(*Booking, *Office)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Booking)
	for i := range nodes {
		fk := nodes[i].OfficeID
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "office_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (bq *BookingQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := bq.ctx.Fields; len(fields) > 0 {
		...
		if bq.withOffice != nil {
			_spec.Node.AddColumnOnce(booking.FieldOfficeID)
		}
		if bq.withRoom != nil {
