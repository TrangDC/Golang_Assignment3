...
import (
	...
	"github.com/techvifyharrydo/Room-Reservation/ent/migrate"
...
func (c *Client) Intercept(interceptors ...Interceptor) {
	...
	c.Office.Intercept(interceptors...)
...
// QueryOffice queries the office edge of a Booking.
func (c *BookingClient) QueryOffice(b *Booking) *OfficeQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, booking.OfficeTable, booking.OfficeColumn),
		)
		fromV = sqlgraph.Neighbors(b.driver.Dialect(), step)
		return fromV, nil
	}
...
// UpdateOneID returns an update builder for the given id.
func (c *OfficeClient) UpdateOneID(id uuid.UUID) *OfficeUpdateOne {
...
// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *OfficeClient) DeleteOneID(id uuid.UUID) *OfficeDeleteOne {
...
func (c *OfficeClient) Get(ctx context.Context, id uuid.UUID) (*Office, error) {
	return c.Query().Where(office.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *OfficeClient) GetX(ctx context.Context, id uuid.UUID) *Office {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryRooms queries the rooms edge of a Office.
func (c *OfficeClient) QueryRooms(o *Office) *RoomQuery {
	query := (&RoomClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := o.ID
		step := sqlgraph.NewStep(
...
// QueryBookings queries the bookings edge of a Office.
func (c *OfficeClient) QueryBookings(o *Office) *BookingQuery {
...
// QueryOffice queries the office edge of a Room.
func (c *RoomClient) QueryOffice(r *Room) *OfficeQuery {
	query := (&OfficeClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := r.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(room.Table, room.FieldID, id),
			sqlgraph.To(office.Table, office.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, room.OfficeTable, room.OfficeColumn),
		)
		fromV = sqlgraph.Neighbors(r.driver.Dialect(), step)
		return fromV, nil
	}
...
type (
	hooks struct {
		Booking, Office, Room, User []ent.Hook
	}
	inters struct {
		Booking, Office, Room, User []ent.Interceptor
	}
)
