...
type NewsResponseGetAll struct {
...
const (
	...
	NewsStatusHidden    NewsStatus = "hidden"
)

var AllNewsStatus = []NewsStatus{
	NewsStatusPublished,
	NewsStatusDraft,
	NewsStatusHidden,
}

func (e NewsStatus) IsValid() bool {
	switch e {
	case NewsStatusPublished, NewsStatusDraft, NewsStatusHidden:
		return true
	}
	return false
}

func (e NewsStatus) String() string {
	return string(e)
}

func (e *NewsStatus) UnmarshalGQL(v interface{}) error {
	str, ok := v.(string)
	if !ok {
		return fmt.Errorf("enums must be strings")
	}
	...
	*e = NewsStatus(str)
	if !e.IsValid() {
		return fmt.Errorf("%s is not a valid NEWS_STATUS", str)
	}
	return nil
}

func (e NewsStatus) MarshalGQL(w io.Writer) {
	fmt.Fprint(w, strconv.Quote(e.String()))
}
