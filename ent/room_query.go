...
func (rq *RoomQuery) QueryOffice() *OfficeQuery {
	query := (&OfficeClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoomQuery) Clone() *RoomQuery {
	...
	return &RoomQuery{
		...
		withOffice:   rq.withOffice.Clone(),
...
func (rq *RoomQuery) WithOffice(opts ...func(*OfficeQuery)) *RoomQuery {
	query := (&OfficeClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Room, error) {
	...
	if query := rq.withOffice; query != nil {
		if err := rq.loadOffice(ctx, query, nodes, nil,
			func(n *Room, e *Office) { n.Edges.Office = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoomQuery) loadOffice(ctx context.Context, query *OfficeQuery, nodes []*Room, init func(*Room), assign func(*Room, *Office)) error {
...
func (rq *RoomQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withOffice != nil {
			_spec.Node.AddColumnOnce(room.FieldOfficeID)
		}
	}
