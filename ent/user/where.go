...
// HasNewsEdges applies the HasEdge predicate on the "news_edges" edge.
func HasNewsEdges() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(NewsEdgesTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, NewsEdgesTable, NewsEdgesColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasNewsEdgesWith applies the HasEdge predicate on the "news_edges" edge with a given conditions (other predicates).
func HasNewsEdgesWith(preds ...predicate.News) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(NewsEdgesInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, NewsEdgesTable, NewsEdgesColumn),
		)
